...
import { Alert, Modal, Pressable, ScrollView, StyleSheet, Text, View, Platform, BackHandler } from 'react-native';
...
export function LockScreen({ route, navigation }: any) {
  ...
  // Use lock store for API-fetched questions
  const lockStore = useLockStore((s) => s);
  const questionState = useLockStore((s) => s.questionState);
  ...
  // Hard Lock: Disable back button on Android
  useEffect(() => {
    if (activeLock?.lockType === 'hard' && Platform.OS === 'android') {
      const backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
        // Prevent back button during Hard Lock
        return true;
      });
  ...
  // Check temp access expiration periodically
  useEffect(() => {
    if (activeLock?.lockType === 'soft' && questionState.tempAccessGranted) {
      const interval = setInterval(() => {
        const isValid = lockStore.checkTempAccess();
        if (!isValid && activeLock) {
          // Temp access expired - reset timer
          Alert.alert('Access Expired', 'Temporary access has expired. Lock is active again.');
        }
      }, 1000);
      
      return () => clearInterval(interval);
    }
  }, [activeLock?.lockType, questionState.tempAccessGranted, lockStore]);
  ...
  async function openChallengePreview() {
    if (!activeLock || activeLock.lockType !== 'soft') return;
    
    // Check if temp access is still valid
    if (lockStore.checkTempAccess()) {
      Alert.alert('Access Granted', 'You have temporary access. Lock will resume after expiration.');
      return;
    }
    ...
    await lockStore.fetchQuestions(challengeDifficulty, count);
  }

  function submitChallenge() {
    const { questions, answers } = questionState;
    ...
    const result = lockStore.submitAnswers();
    recordChallengeResult(result.success);
    
    if (!result.success) {
      // Incorrect answer - reset timer (extend lock)
      Alert.alert(
        'Challenge Failed',
        `You got ${result.correctCount}/${result.totalCount} correct. Lock timer will continue.`
      );
      lockStore.resetQuestionState();
      setChallengeVisible(false);
    } else {
      // Correct answer - grant temporary access
      const tempAccessDuration = 15 * 60 * 1000; // 15 minutes
      lockStore.grantTempAccess(tempAccessDuration);
      
      Alert.alert(
        'Challenge Passed!',
        `You got all ${result.totalCount} correct. Temporary access granted for ${Math.floor(tempAccessDuration / 60000)} minutes.`
      );
      setChallengeVisible(false);
    }
  }
          ...
          <GlassCard style={styles.activeCard} intensity={44}>
            ...
            {activeLock.lockType === 'soft' ? (
              <View style={styles.activeActions}>
                {questionState.tempAccessGranted && lockStore.checkTempAccess() ? (
                  <View style={styles.tempAccessBadge}>
                    <Ionicons name="checkmark-circle" size={16} color={Colors.blue} />
                    <Text style={styles.tempAccessText}>
                      Temporary Access Active ({formatRemaining((questionState.tempAccessExpiresAt ?? Date.now()) - Date.now())})
                    </Text>
                  </View>
                ) : (
                  <Pressable onPress={openChallengePreview} style={({ pressed }) => [styles.smallAction, pressed && { opacity: 0.8 }]}>
                    ...
                    <Text style={styles.smallActionText}>Solve Challenge</Text>
                  </Pressable>
                )}
                <Pressable
                  onPress={() =>
              ...
              <Text style={styles.hardNote}>No override. Timer must expire. Back button disabled.</Text>
            )}
          </GlassCard>
        ) : null}
          ...
          <GlassCard intensity={64} style={styles.challengeCard}>
            ...
            {questionState.isLoading ? (
              <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>Loading SAT questions...</Text>
              </View>
            ) : questionState.error && questionState.questions.length === 0 ? (
              <View style={styles.errorContainer}>
                <Ionicons name="warning-outline" size={24} color={Colors.textDim} />
                <Text style={styles.errorText}>{questionState.error}</Text>
                <Text style={styles.errorSubText}>Using local fallback questions.</Text>
              </View>
            ) : questionState.questions.length === 0 ? (
              <View style={styles.emptyContainer}>
                <Text style={styles.emptyText}>No questions available. Please try again.</Text>
              </View>
            ) : (
              <>
                {questionState.error && (
                  <View style={styles.warningBanner}>
                    ...
                    <Text style={styles.warningText}>Using fallback questions. {questionState.error}</Text>
                  </View>
                )}
                <ScrollView style={{ maxHeight: 420 }} showsVerticalScrollIndicator={false}>
                  {questionState.questions.map((q, idx) => {
                    const picked = questionState.answers[q.id] ?? null;
                    return (
                      <View key={q.id} style={styles.qBlock}>
                        ...
                        {q.choices.map((c, i) => {
                          ...
                          return (
                            <Pressable
                              key={`${q.id}-${i}`}
                              onPress={() => lockStore.setAnswer(q.id, i)}
                              style={({ pressed }) => [
                                styles.choice,
                            ...
                            >
                              <Text style={[styles.choiceText, isPicked && { color: Colors.text }]}>{c}</Text>
                            </Pressable>
                          );
                        })}
                      </View>
                ...
                <PrimaryButton label="Submit" onPress={submitChallenge} disabled={questionState.questions.length === 0} />
              </>
            )}
          </GlassCard>
        </View>
...
const styles = StyleSheet.create({
  ...
  warningBanner: {
    ...
    backgroundColor: 'rgba(74,141,255,0.10)',
    borderWidth: 1,
    borderColor: 'rgba(74,141,255,0.25)',
    marginBottom: 12,
  },
  warningText: {
    flex: 1,
